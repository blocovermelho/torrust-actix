        ...
        return match ip {
                ...
                if announce_unwrapped.left != 0 {
                    ...
                    if seeds.is_some() {
                        for (_, torrent_peer) in seeds.unwrap().iter() {
                            let peer_pre_parse = match torrent_peer.peer_addr_v4.unwrap().ip().to_string().parse::<Ipv4Addr>() {
                                Ok(ip) => { ip }
                                Err(e) => {
                                    error!("[IPV4 Error] {} - {}", torrent_peer.peer_addr_v4.unwrap().ip().to_string(), e.to_string());
                                    return HttpResponse::Ok().content_type(ContentType::plaintext()).body(ben_map! {
                                        "failure reason" => ben_bytes!(e.to_string())
                                    }.encode());
                                }
            ...
            IpAddr::V6(_) => {
                if announce_unwrapped.left != 0 {
                    ...
                    if seeds.is_some() {
                        for (_, torrent_peer) in seeds.unwrap().iter() {
                            let peer_pre_parse = match torrent_peer.peer_addr_v6.unwrap().ip().to_string().parse::<Ipv6Addr>() {
                                Ok(ip) => { ip }
                                Err(e) => {
                                    error!("[IPV6 Error] {} - {}", torrent_peer.peer_addr_v6.unwrap().ip().to_string(), e.to_string());
                                    return HttpResponse::Ok().content_type(ContentType::plaintext()).body(ben_map! {
                                            "failure reason" => ben_bytes!(e.to_string())
                                        }.encode());
                                }
                ...
                if seeds.is_some() {
                    for (peer_id, torrent_peer) in seeds.unwrap().iter() {
                        peers_list_mut.push(ben_map! {
                            ...
                            "ip" => ben_bytes!(torrent_peer.peer_addr_v4.unwrap().ip().to_string()),
                            "port" => ben_int!(torrent_peer.peer_addr_v4.unwrap().port() as i64)
                        });
                    }
                }
            }
            ...
            if announce_unwrapped.left != 0 {
                ...
                if seeds.is_some() {
                    for (peer_id, torrent_peer) in seeds.unwrap().iter() {
                        peers_list_mut.push(ben_map! {
                            "peer id" => ben_bytes!(peer_id.to_string()),
                            "ip" => ben_bytes!(torrent_peer.peer_addr_v6.unwrap().ip().to_string()),
                            "port" => ben_int!(torrent_peer.peer_addr_v6.unwrap().port() as i64)
                        });
                    }
                }
            }
